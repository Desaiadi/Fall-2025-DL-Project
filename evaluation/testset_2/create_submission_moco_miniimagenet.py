"""
Create Kaggle Submission with YOUR MoCo v3 Model - Mini-ImageNet
================================================================

This script uses YOUR trained SSL model (MoCo v3) for feature extraction
and KNN classifier for predictions on Mini-ImageNet dataset.

Mini-ImageNet: 100 object classes (subset of ImageNet)
...
def train_knn_classifier(train_features, train_labels, val_features, val_labels, k_values=[1, 5, 10, 20, 50, 100]):
    """
...
def create_submission(test_features, test_filenames, classifier, output_path, num_classes=100):
    """
    ...
    Args:
        ...
        num_classes: Number of classes (100 for Mini-ImageNet)
    """
    ...
    print(f"  Unique classes predicted: {len(class_dist)}/{num_classes}")
    ...
    assert submission_df['class_id'].max() <= (num_classes - 1), f"Invalid class_id > {num_classes-1}"
    assert submission_df.isnull().sum().sum() == 0, "Missing values found!"
    print(f"✓ Submission format is valid for {num_classes} classes (Mini-ImageNet)!")
...
def main():
    parser = argparse.ArgumentParser(description='Create Kaggle Submission with YOUR MoCo v3 Model - Mini-ImageNet')
    ...
    parser.add_argument('--num_classes', type=int, default=100,
                        help='Number of classes (100 for Mini-ImageNet)')
    ...
    print(f"Dataset: Mini-ImageNet ({args.num_classes} classes)")
    ...
    # Verify class count
    actual_classes = train_df['class_id'].nunique()
    if actual_classes != args.num_classes:
        print(f"\n⚠️  Warning: Expected {args.num_classes} classes, found {actual_classes}")
        print(f"  Updating num_classes to {actual_classes}")
    ...
    print("="*60)
    print(f"\nYour MoCo v3 model (epoch 49) evaluated on Mini-ImageNet!")
    print(f"Dataset: Mini-ImageNet ({args.num_classes} classes)")
