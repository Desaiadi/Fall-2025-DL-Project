...
class FeatureExtractor:
    """Load YOUR trained SSL model checkpoint"""
    ...
    def __init__(self, checkpoint_path, device='cuda'):
        ...
        Args:
            checkpoint_path: Path to your .pth file
            device: 'cuda' or 'cpu'
        """
        ...
        missing, unexpected = self.model.load_state_dict(student_state, strict=False)
        print(f"Loaded model - Missing: {len(missing)}, Unexpected: {len(unexpected)}")
        
        # FREEZE encoder (required for competition!)
        self.model.eval()
