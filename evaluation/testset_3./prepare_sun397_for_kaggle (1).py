...
def create_kaggle_dataset(
    ...
    resolution=96,
    seed=42,
    batch_size=500  # ← ADD THIS PARAMETER
):
    """
    Create Kaggle competition dataset from SUN397 HuggingFace dataset
    Process images in BATCHES to avoid memory crash!
    """
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    print("\n" + "="*60)
    print(f"Creating SUN397 Kaggle dataset (BATCHED)")
    print(f"  Classes: {len(class_info)}")
    print(f"  Images: {len(hf_dataset)}")
    print(f"  Resolution: {resolution}x{resolution}")
    ...
    # Create INDICES only (not images!)
    ...
    def save_split(split_indices, split_name):
        split_dir = output_dir / split_name
        split_dir.mkdir(parents=True, exist_ok=True)
        
        metadata = []
        
        print(f"\nSaving {split_name} images (batches of {batch_size})...")
        
        # Process in batches
        for batch_start in tqdm(range(0, len(split_indices), batch_size), desc=f"{split_name} batches"):
            batch_end = min(batch_start + batch_size, len(split_indices))
            batch_dataset_indices = split_indices[batch_start:batch_end]
            ...
            for i, dataset_idx in enumerate(batch_dataset_indices):
                try:
                    ...
                    # Load ONE image
                    item = hf_dataset[int(dataset_idx)]
                    img = item['image'].convert('RGB')
                    ...
                    # Save
                    filename = f"{new_idx:06d}_class{item['label']:03d}.jpg"
                    img.save(split_dir / filename, quality=85)
                    ...
                    print(f"\nWarning: Failed to process image {dataset_idx}: {e}")
                    continue
